    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="flex h-[calc(100vh-56px)]">
        ...
        <div className={`w-[460px] shrink-0 border-l border-slate-200 bg-white overflow-y-auto ${showPreview ? 'block' : 'hidden lg:block'}`}>
          ...
          <div className="p-6">
            <InvoicePreview from={from} to={to} details={details} items={items} subtotal={subtotal} taxTotal={taxTotal} discountAmount={discountAmount} grandTotal={grandTotal} notes={notes} paymentInfo={paymentInfo} fmt={fmt} templateId={params.templateId} />
          </div>
        </div>
      </div>
    </div>
...
function InvoicePreview({ from, to, details, items, subtotal, taxTotal, discountAmount, grandTotal, notes, paymentInfo, fmt, templateId }: PreviewProps) {
  ...
  const accent = accents[templateId] ?? '#2563eb'

  return (
    <div className="bg-white rounded-lg shadow-md overflow-hidden text-xs" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      <div className="px-6 py-4 grid grid-cols-2 gap-4 bg-slate-50">
        <div>
          <p className="font-bold text-slate-400 uppercase tracking-wide text-[9px] mb-1">Bill to</p>
        ...
        <div className="text-right">
          <p className="font-bold text-slate-400 uppercase tracking-wide text-[9px] mb-1">Details</p>
        ...
        <table className="w-full">
          <thead>
            <tr style={{ borderBottom: `1px solid ${accent}30` }}>
              <th className="text-left pb-2 font-semibold text-slate-400 text-[9px] uppercase tracking-wide">Description</th>
              <th className="text-right pb-2 font-semibold text-slate-400 text-[9px] uppercase tracking-wide">Qty</th>
              <th className="text-right pb-2 font-semibold text-slate-400 text-[9px] uppercase tracking-wide">Price</th>
              <th className="text-right pb-2 font-semibold text-slate-400 text-[9px] uppercase tracking-wide">Total</th>
            </tr>
          </thead>
          <tbody>
            {items.filter((i) => i.description || i.unit_price > 0).map((item) => (
              <tr key={item.id} className="border-b border-slate-50">
                ...
                <td className="py-1.5 text-right text-slate-600">{fmt(item.unit_price)}</td>
                <td className="py-1.5 text-right font-medium text-slate-900">{fmt(item.quantity * item.unit_price)}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {(notes || paymentInfo) && (
        <div className="px-6 pb-6 grid grid-cols-2 gap-4 border-t border-slate-100 pt-4">
          {notes && <div><p className="font-bold text-[9px] uppercase tracking-wide text-slate-400 mb-1">Notes</p><p className="text-slate-600">{notes}</p></div>}
          {paymentInfo && <div><p className="font-bold text-[9px] uppercase tracking-wide text-slate-400 mb-1">Payment</p><p className="text-slate-600 whitespace-pre-line">{paymentInfo}</p></div>}
        </div>
      )}

      <div className="px-6 py-3 text-center text-[9px] text-slate-400" style={{ borderTop: `2px solid ${accent}20` }}>
