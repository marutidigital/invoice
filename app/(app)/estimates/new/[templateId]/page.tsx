    ...
    async function load() {
      ...
      if (prof?.active_business_id) {
        const { data: b } = await supabase
          .from('businesses')
          .select('*')
          .eq('id', prof.active_business_id)
          .single()
        biz = b
        setActiveBiz(b)
      }

      // Fetch clients for active business
      const { data: c } = await supabase
        .from('clients')
        .select('*')
        .eq('business_id', prof?.active_business_id)
        .order('name')
      setClients(c ?? [])
      ...
      if (targetEstimateId) {
        ...
        if (biz) {
          setFrom({
            ...
            email: biz.email ?? '',
            phone: biz.phone ?? '',
            address: [biz.address, biz.city, biz.state, biz.postcode, biz.country].filter(Boolean).join(', '),
